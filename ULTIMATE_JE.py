...
# Analytics and Database imports
from database import SessionLocal, User, ProblemSolved, engine, Base
...
from advanced_features import (
    dashboard_command, weaknesses_command, achievements_command,
    leaderboard_command, practice_command, multidebate_command,
    get_or_create_user, init_advanced_features
)

# Complete command handlers for all 52 features
from complete_command_handlers import NEW_COMMANDS
...
logger.info(f"✅ Loaded {len(GEMINI_API_KEYS)} API keys for rotation")

# Global API key rotation index (rotates through all 5 keys for reliability)
# Used by: call_gemini(), multi-agent debate system, content generator
current_key_index = 0
# Cache file path - ensure directory exists
CACHE_DIR = "/app/data" if os.path.exists("/app") else "."
CHEMISTRY_CACHE_FILE = os.path.join(CACHE_DIR, "chemistry_cache.json")
...
CHEMISTRY_SOURCES = {
    # ========== REAL GITHUB SOURCES - FIXED BRANCH & ORG ==========
    # Global-Chem moved to new org and uses 'development' branch
    "functional_groups": "https://raw.githubusercontent.com/Global-Chem/global-chem/development/global_chem/functional_groups/functional_groups.json",
    "common_r_groups": "https://raw.githubusercontent.com/Global-Chem/global-chem/development/global_chem/common_R_groups/common_R_groups.json",
    "amino_acids": "https://raw.githubusercontent.com/Global-Chem/global-chem/development/global_chem/amino_acids/amino_acids.json",
    "common_solvents": "https://raw.githubusercontent.com/Global-Chem/global-chem/development/global_chem/common_solvents/common_solvents.json",
    "named_reactions": "https://raw.githubusercontent.com/Global-Chem/global-chem/development/global_chem/named_reactions/named_reactions.json",
    
    # OpenChemistry - Certified open source project
    "molecules_fragments": "https://raw.githubusercontent.com/OpenChemistry/molecules/master/fragments.json",
    
    # Expanded Global-Chem sources
    "common_warheads": "https://raw.githubusercontent.com/Global-Chem/global-chem/development/global_chem/common_warheads/common_warheads.json",
    "vitamins": "https://raw.githubusercontent.com/Global-Chem/global-chem/development/global_chem/vitamins/vitamins.json",
    "rings_in_drugs": "https://raw.githubusercontent.com/Global-Chem/global-chem/development/global_chem/rings_in_drugs/rings_in_drugs.json",
    "iupac_blue_book": "https://raw.githubusercontent.com/Global-Chem/global-chem/development/global_chem/iupac_blue_book/iupac_blue_book.json",
    "peptide_bases": "https://raw.githubusercontent.com/Global-Chem/global-chem/development/global_chem/peptide_bases/peptide_bases.json",
...
def save_cache():
    try:
        cache_dir = os.path.dirname(CHEMISTRY_CACHE_FILE)
        if cache_dir:  # Only create directory if path has a directory component
            os.makedirs(cache_dir, exist_ok=True)
        with open(CHEMISTRY_CACHE_FILE, 'w') as f:
            json.dump(chemistry_knowledge_base, f, indent=2)
        logger.info(f"💾 Cache saved to {CHEMISTRY_CACHE_FILE}")
    except Exception as e:
        logger.warning(f"⚠️ Cache save warning: {e} (will use in-memory cache)")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Track in both old system and new database
    if track_new_user(user_id, username):
        await notify_new_user(user_id, username, context)
    
    # Get or create user in new database system
    ...
    await update.message.reply_text(
        f"🔬 *ULTIMATE CHEMISTRY BOT - ADVANCED EDITION*\n\n"
        f"📚 Knowledge Sources: {kb_count}+ databases {status}\n\n"
        f"*CORE FEATURES:*\n📸 Problem solving with PDF\n💬 Text queries\n🌙 Dark mode\n\n"
        ...
        f"*✨ NEW - PERSONALIZED FEATURES:*\n📊 /dashboard - Your performance\n"
        f"⚠️ /weaknesses - Weak topics\n🏆 /achievements - Your badges\n"
        f"🏅 /leaderboard - Rankings\n🎯 /practice <topic> - AI problems\n"
        f"🤖 /multidebate - Multi-agent mode\n\n"
...
async def startup():
    logger.info("="*70)
    logger.info("🔬 ULTIMATE CHEMISTRY BOT - ADVANCED EDITION STARTUP")
    logger.info("="*70)
    ...
    try:
        init_advanced_features(GEMINI_API_KEYS)
        logger.info("✅ Advanced features initialized")
    except Exception as e:
        logger.warning(f"⚠️ Advanced features initialization partial: {e}")

    logger.info("="*70)
    logger.info(f"✅ Knowledge Sections: {len(chemistry_knowledge_base)}")
    logger.info(f"✅ API Keys: {len(GEMINI_API_KEYS)}")
    logger.info(f"✅ Admin: {ADMIN_USERNAME}")
    logger.info(f"✅ Database: SQLite initialized")
    logger.info(f"✅ Features: Core + Visualization + Personalized AI")
    logger.info("="*70)
...
def main():
    ...
    # Advanced features (50+ feature system)
    app.add_handler(CommandHandler("dashboard", dashboard_command))
    app.add_handler(CommandHandler("weaknesses", weaknesses_command))
    app.add_handler(CommandHandler("achievements", achievements_command))
    app.add_handler(CommandHandler("leaderboard", leaderboard_command))
    app.add_handler(CommandHandler("practice", practice_command))
    app.add_handler(CommandHandler("multidebate", multidebate_command))
    
    # Register all 14 new commands from complete_command_handlers
    for cmd_name, cmd_handler in NEW_COMMANDS.items():
